...
// AddWord -- добавляет слова комментариев за концом модуля
func (sf TComment) AddWord(poolWord []*word.TWord) int {
	sf.poolWord = append(sf.poolWord, poolWord...)
	//log.Printf("TComment.AddWord(): len=%v", len(sf.poolWord))
