...
type ИМодуль interface {
	...
	Обработать(types.СИсхФайл) error
}

//ИМодули -- интерфейс к глобальному объекту модулей
type ИМодули interface {
	МодульДоб(пСлово IWord, путьМодуль types.UWord) error //добавляет новый модуль в глобальный справочник модулей
}

//Тип глобального объекта экспортироваться не будет
type тМодули struct {
	спрМодули map[int]types.ИМодульОпис
}

var (
	//Модули -- Объект управляет модулями при компиляции
	Модули types.ИМодули
)
...
func модулиНов() (модули *тМодули, ош error) {
	...
	модули.спрМодули = make(map[int]types.ИМодульОпис)
	return модули, nil
}

//МодульДоб -- добавляет новый модуль в список
func (сам *тМодули) МодульДоб(пСлово types.IWord, пПуть types.UWord) (ош error) {
	if пСлово == nil {
	...
	{              //1. Получить полный список хранимых имён модулей
		for _, модуль := range сам.спрМодули {
			if модуль.Имя() == types.СМодульИмя(пСлово.Слово()) {
				бЕсть = true
				break
			}
		}
