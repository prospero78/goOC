...
//UWordNumМодуль -- специальный целочисленный тип для хранения номера слова модуля
type UWordNumМодуль int

//ТСекцияМодуль -- предоставляет тип, содержащий слова модуля
type ТСекцияМодуль struct {
	*ТСекция
	модульИмя     СМодульИмя
	слово         мСлово.IWord // слово в котором хранится имя модуля
	сканер        мСкан.IScanner
...
func (сам *ТСекцияМодуль) Обработать(пФайлИмя мИф.UFileSourceName) (ош error) {
...
func (сам *ТСекцияМодуль) _МодульКонецНайти() (словоНомерКонец UWordNumМодуль, ош error) {
	мКонс.Конс.Отладить("ТСекцияМодуль._МодульКонецНайти()")
	цСчётОбр := UWordNumМодуль(len(сам.СловаМодуля()) - 1)
	for цСчётОбр = UWordNumМодуль(len(сам.СловаМодуля()) - 1); цСчётОбр >= 0; цСчётОбр-- {
...
func (сам *ТСекцияМодуль) _МодульКонецОбрезать(пСчётОбр UWordNumМодуль) error {
	if !((0 <= пСчётОбр) && (пСчётОбр+2 <= мИнт.UWordNumМодуль(len(сам.словаМодуля)-1))) {
		return мФмт.Errorf("ТСекцияМодуль._МодульКонецобрезать(): нарушена структура модуля?\n")
	}
	//Всё, что за точкой -- отправить в комментарии
	индекс := UWordNumМодуль(0)
	for индекс = UWordNumМодуль((len(сам.словаМодуля))); индекс >= пСчётОбр; индекс-- {
...
func (сам *ТСекцияМодуль) _МодульОдинПроверить() (ош error) {
	...
	for адр := 0; адр < len(сам.СловаМодуля()); адр++ {
		ключ := сам.СловаМодуля()[мИнт.UWordNumМодуль(адр)]
		//мФмт.Printf("%v\n", ключ)
