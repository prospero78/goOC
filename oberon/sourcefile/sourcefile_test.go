...
func TestИсходникФайл(тест *мТест.T) {
	_Позитив := func() {
		{ //1 Создание файла исходника
			...
			if исхФайл, ош = ИсхФайлНов(стрФайл); ош != nil {
		...
		{ //3 Считывание файла исходника
			...
			if исхФайл.Размер() != 2713 {
				тест.Errorf("п3.1 ОШИБКА при проверке размера текста исходника(2713), размер=[%v]\n", исхФайл.Размер())
			}
		}
		{ //4 Проверка литеры в указанной позиции
			...
			if лит != "ж" {
				тест.Errorf("п4.2 ОШИБКА при проверке литеры в позиции(36, ж), лит=[%v]\n", лит)
			}
		}
		{ //5 Печать исходника
