...
type ИПулИсхСтроки interface {
	...
	Строка(мНс.ССтрНомер) (мИсс.СИсхСтрокаСтр, error)
...
func (сам *тПулИсхСтроки) Строка(пНомер мНс.ССтрНомер) (стр мИсс.СИсхСтрокаСтр, ош error) {
	defer сам.блок.RUnlock()
	сам.блок.RLock()
	for _, строка := range сам.пулСтроки {
		if строка.Номер() == пНомер {
			return строка.Строка(), nil
		}
	}
	return "", мФмт.Errorf("тПулИсхСтроки.Строка(): строки с номером [%v] не существует\n", пНомер)
}
