...
type ТМодуль struct {
	...
	сканер      мИнт.ИСканер
	секцКоммент мИнт.ИСекцияКоммент //Секция комментариев
}
...
func (сам *ТМодуль) _ИмяПроверить() (ош error) {
	...
	if слово.ЕслиИмяСтрого() {
		...
		стрИсх := слово.Строка()
		return мФмт.Errorf("ТМодуль._ИмяПроверить(): такое имя модуля запрещено, имя=[%v], строка=[%v]\n", стрСлово, стрИсх)
	}
	if сам.слова, ош = мИнт.СловаОбрезать(сам.слова); ош != nil {
		return мФмт.Errorf("ТМодуль._ИмяПроверить(): ошибка при обрезке слов модуля\n\t%v", ош)
	}
...
func (сам *ТМодуль) _РазделительПроверить() (ош error) {
	мКонс.Конс.Отладить("ТМодуль._РазделительПроверить()")
	слово := сам.слова[0]
	стрСлово := слово.Слово()
	стрИсх := слово.Строка()
	if стрСлово != ";" {
		return мФмт.Errorf("ТМодуль._РазделительПроверить(): ошибка в окончании названия модуля, слово=[%v], строка=[%v]\n",
			стрСлово, стрИсх)
	}
	if сам.слова, ош = мИнт.СловаОбрезать(сам.слова); ош != nil {
		return мФмт.Errorf("ТМодуль._РазделительПроверить(): ошибка пи обрезании слов модуля\n\t%v", ош)
	}
	return nil
}

func (сам *ТМодуль) _МодульКонецНайти() (ош error) {
	мКонс.Конс.Отладить("ТМодуль._МодульКонецНайти()")
		...
		if стрТочка == "." {
			...
			for _, строкаКонец := range мСлово.КсКонец {
				...
				if стрКонец == строкаКонец {
					...
					return мФмт.Errorf("ТМодуль._КонецМодульНайти(): имя модуля в начале и в конце модуля не совпадают\n\t%v %v",
						стрИмя, стрИмя2)
				}
			}
		}
	...
	if цСчётОбр == 0 { // Гарантированная ошибка. Так быть не может.
		return мФмт.Errorf("ТМодуль._КонецМодуль_Найти(): нет завершающей точки в модуле")
	}
	return nil
}
...
func (сам *ТМодуль) _МодульОдинПроверить() (ош error) {
	мКонс.Конс.Отладить("ТМодуль._МодульОдинПроверить()")
	for индекс := range сам.слова {
		...
		for индекс2 := range мСлово.КсМодуль {
			...
			if стрМодуль == мСлово.КсМодуль[индекс2] {
				return мФмт.Errorf("ТМодуль._МодульОдинПроверить(): MODULE встречается больше одного раза")
			}
		}
	}
	return nil
...
func (сам *ТМодуль) _МодульНачалоПроверить() (ош error) {
	мКонс.Конс.Отладить("ТМодуль._МодульНачало_Проверить()")
	бРез := false
	словоМОДУЛЬ := сам.слова[0]
	...
	if !бРез {
		return мФмт.Errorf("ТМодуль._МодульНачалоПроверить(): модуль не начинается с MODULE")
	}
	if сам.слова, ош = мИнт.СловаОбрезать(сам.слова); ош != nil {
		return мФмт.Errorf("ТМодуль._МодульНачалоПроверить(): ошибка при обрезании слов модуля\n\t%v", ош)
	}
	return nil
}
