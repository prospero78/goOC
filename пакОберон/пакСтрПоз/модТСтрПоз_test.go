...
func TestПозСтр(тест *мТест.T) {
	_Позитив := func() {
		...
		{ //6
			тест.Logf("п6 Сброс позиции в строке\n")
			позИзм.Сброс()
			if поз.Знач() != 0 {
				тест.Errorf("п6.1 ОШИБКА позиция в строке должен быть 0, знач=[%v]\n", поз)
			}
			if позИзм.String() != "0" {
				тест.Errorf("п6.2 ОШИБКА строковое представление позиции в строке должен быть (0), стр=[%v]\n", поз)
			}
		}
	}
	_Негатив := func() {
