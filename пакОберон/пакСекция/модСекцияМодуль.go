...
func (сам *ТСекцияМодуль) _МодульКонецНайти() (словоНомерКонец мИнт.ССловоНомерМодуль, ош error) {
	мКонс.Конс.Отладить("ТСекцияМодуль._МодульКонецНайти()")
	цСчётОбр := len(сам.СловаМодуля()) - 1
	for цСчётОбр := len(сам.СловаМодуля()) - 1; цСчётОбр >= 0; цСчётОбр-- {
		словоТочка := сам.СловаМодуля()[цСчётОбр]
		// Нашли конечную точку?
		стрТочка := словоТочка.Слово()
		ок, ош := мКлючи.Ключи.Проверить(".", мИнт.СКлюч(стрТочка))
		if ош != nil {
		...
		if мИнт.СМодульИмя(стрИмя) == сам.МодульИмя() {
			continue
		}
		...
		return словоКонец, nil
	}
	return мИнт.СловоНомерМодуль(цСчётОбр), мФмт.Errorf("ТСекцияМодуль._КонецМодуль_Найти(): нет завершающей точки в модуле")
}

// Отбросить всё, что за END <name_module>.-- загнать в секцию комментариев
func (сам *ТСекцияМодуль) _МодульКонецОбрезать(пСчётОбр мИнт.ССловоНомерМодуль)error {
	if !(пСчётОбр < 0 && пСчётОбр + 2 <len(сам.словаМодуля)-1){
		return мФмт.Errorf("ТСекцияМодуль._МодульКонецобрезать(): нарушена структура модуля?\n")
	}
	for индекс := пСчётОбр; индекс <= (пСчётОбр + 2); индекс++ {
		if инде
