...
type ТСекцияМодуль struct {
	*ТСекция
	...
	секцияИмпорт  мИнт.ИСекцияИмпорт
}
...
func (сам *ТСекцияМодуль) _МодульКонецНайти() (словоНомерКонец мИнт.ССловоНомерМодуль, ош error) {
	...
	цСчётОбр := мИнт.ССловоНомерМодуль(len(сам.СловаМодуля()) - 1)
	for цСчётОбр = мИнт.ССловоНомерМодуль(len(сам.СловаМодуля()) - 1); цСчётОбр >= 0; цСчётОбр-- {
...
func (сам *ТСекцияМодуль) _МодульКонецОбрезать(пСчётОбр мИнт.ССловоНомерМодуль) error {
	...
	индекс := мИнт.ССловоНомерМодуль(0)
	for индекс = мИнт.ССловоНомерМодуль((len(сам.словаМодуля))); индекс >= пСчётОбр; индекс-- {
