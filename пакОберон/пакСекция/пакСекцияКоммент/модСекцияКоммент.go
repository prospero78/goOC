...
type ИСекцияКоммент interface {
	...
	СловоДоб(мСлово.ИСлово) //Нужно для добавления слов за концом модуля
}
...
func СекцияКомментНов() (коммент ИСекцияКоммент, ош error) {
	...
	_секция.слова = make(map[мСек.ССловоНомер]мСлово.ИСлово)
	return _секция, nil
}
...
func (сам *тСекцияКоммент) Слова() (слова map[мСек.ССловоНомер]мСлово.ИСлово) {
	return сам.слова
}

// СловаУст -- устанавливает список слов секции
func (сам *тСекцияКоммент) СловаУст(пСлова map[мСек.ССловоНомер]мСлово.ИСлово) error {
	if пСлова == nil {
...
func (сам *тСекцияКоммент) СловоДоб(пСлово мСлово.ИСлово) {
	сам.слова[мСек.ССловоНомер(len(сам.слова))] = пСлово
}

//Обработать -- главный обработчик секции
func (сам *тСекцияКоммент) _Обработать() (ош error) {
	цУровень := 0
	спрКоммент := make(map[мСек.ССловоНомер]мСлово.ИСлово)
	спрМодуль := make(map[мСек.ССловоНомер]мСлово.ИСлово)
	адрМодуль := мСек.ССловоНомер(0)
	адрКоммент := мСек.ССловоНомер(0)
	цНомерМодуль := мСек.ССловоНомер(0)
...
func (сам *тСекцияКоммент) СловаДругие() map[мСек.ССловоНомер]мСлово.ИСлово {
	return сам.словаВсе
}
