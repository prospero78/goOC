...
type ИКлюч interface {
	Синонимы() map[int]СКлюч //Возвращает список синонимов ключа
}

//ТКлюч -- предоставляет обработку ключевых (предопределённых) слов
type ТКлюч struct {
	спрКлючи map[int]СКлюч //Синонимы ключевого слова
}

//КлючНов -- возвращает ссылку на новое ключевое слово
func КлючНов(пКлюч СКлюч) (ключ *ТКлюч, ош error) {
	ключ = &ТКлюч{}
	if ключ == nil {
		return nil, мФмт.Errorf("КлючНов(): нет памяти под новое ключевое слово?")
	}
	ключ.спрКлючи = make(map[int]СКлюч)
...
func (сам *ТКлюч) Доб(пКлюч СКлюч) error {
	if пКлюч == "" {
...
func (сам *ТКлюч) ЕслиСовпал(пКлюч СКлюч) (ок bool, ош error) {
	if пКлюч == "" {
