...
//ИнитУст -- устанавливает единственный раз номер строки исходника
func (сам *тСтрНомер) ИнитУст(пНомер ССтрНомер) (ош error) {
	defer сам.блок.Unlock()
	сам.блок.Lock()
	if сам.бУст {
		return мФмт.Errorf("тСтрНомер.ИнитУст(): номер строки уже установлен (%v), пНомер=[%v]\n", сам.знач, пНомер)
	}
	if ош = сам._Уст(пНомер); ош != nil {
		return мФмт.Errorf("тСтрНомер.ИнитУст(): ОШИБКА при установке значения номера строки\n\t%v", ош)
	}
	сам.бУст = true
	return nil
}
...
func (сам *тСтрНомер) Уст(пНомер ССтрНомер) (ош error) {
	defer сам.блок.Unlock()
	сам.блок.Lock()
	return сам._Уст(пНомер)
}

//Потокобезопасная установка значения номера строки
func (сам *тСтрНомер) _Уст(пНомер ССтрНомер) (ош error) {
	if пНомер <= 0 {
