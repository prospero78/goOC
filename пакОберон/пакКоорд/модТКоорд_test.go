...
func TestКоорд(тест *мТест.T) {
	_Позитив := func() {
		{ //10 Создание ИКоорд
			тест.Logf("п10 Создание ИКоорд\n")
			if коорд, ош = КоордНов(номСтр, позСтр); ош != nil {
				тест.Errorf("п10.1 ОШИБКА при создании ИКоорд\n\t%v", ош)
			}
			if коорд == nil {
	...
	_Негатив := func() {
		{ //5 Создание с неправильной строкой
			тест.Logf("н5 Создание ИКоорд с нправильным номером строки\n")
			if коорд, ош = КоордНов(0, позСтр); ош == nil {
				тест.Errorf("н5.1 ОШИБКА при создании ИКоорд\n")
			}
			if коорд != nil {
		...
		{ //6 Создание с неправильной позицией в строке
			тест.Logf("н6 Создание ИКоорд с нправильным номером строки\n")
			if коордИзм, ош = КоордИзмНов(номСтр, -1); ош == nil {
				тест.Errorf("н6.1 ОШИБКА при создании ИКоорд\n")
			}
			if коордИзм != nil {
