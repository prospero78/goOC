...
func TestЛит(тест *мТест.T) {
	_Провер := func(пЛит СЛит, пКласс СЛитКласс) {
		тест.Logf("_Провер(): пЛит=%v пКласс=%v\n", пЛит, пКласс)
		знач := лит.Получ()
		if знач != пЛит {
			тест.Errorf("_Провер(): ОШИБКА знач(%v)!=пЛит(%v)\n", знач, пЛит)
		}
	}
	_Создать := func() {
	...
	_Негатив := func() {
		...
		_ПустоУст := func() {
			...
			defer func() {
				if паника := recover(); паника == nil {
					тест.Errorf("_ПустоУст(): ОШИБКА при генерации пустой паники\n")
				}
			}()
			лит.Уст("")
		}
		_КриваяЛит := func() {
			...
			defer func() {
				if паника := recover(); паника == nil {
					тест.Errorf("_КриваяЛит(): ОШИБКА при генерации пустой паники\n")
				}
			}()
			лит.Уст("\t")
		}
