...
func (сам *ТуИсхФайл) Лит(пПоз int) (лит rune) {
	if пПоз < 0 {
	...
	return сам.текст[пПоз]
}

// Возвращает полность исходный текст в отдельном срезе рун
func (сам *ТуИсхФайл) Исходник() (текст []rune) {
	текст = make([]rune, cap(сам.текст))
	copy(сам.текст, текст)
	return текст
}
