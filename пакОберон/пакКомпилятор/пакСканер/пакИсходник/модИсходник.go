...
type ТИсходник struct {
	...
	конс         *пакКонс.ТКонсоль    // ссылка на глобальную консоль
}
...
func (сам *ТИсходник) Обработать(пИмяФайла string) (ош error) {
	сам.конс.Отладить("ТИсходник.Обработать()")
...
func (сам *ТИсходник) _НаСловаРазделить() (ош error) {
	...
	for {
		if сам.бСловаГотово {
			сам.конс.Отладить("ТИсходник._СловоДобав(): всего слов: " + пакФмт.Sprintf("%d", len(сам.СловаМодуля)))
			break
		} else {
			сам._СловоВыделить()
		}
