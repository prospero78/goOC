...
//СекцииНов -- возвращает новый экземпляр типа секций (с секциями для хранения слов)
func СекцииНов() (секции *ТСекции, ош error) {
	мКонс.Конс.Отладить("СекцииНов()")
	секции = &ТСекции{}
	if секции.коммент, ош = мКоммент.Новый(); ош != nil {
		return nil, мФмт.Errorf("СекцииНов(): ошибка при создании ТКоммент\n\t%v", ош)
	}
	if секции.модуль, ош = мМодуль.Новый(); ош != nil {
		return nil, мФмт.Errorf("СекцииНов(): ошибка при создании ТМодуль\n\t%v", ош)
	}
	if секции.импорт, ош = мИмпорт.ИмпортНов(); ош != nil {
		return nil, мФмт.Errorf("СекцииНов(): ошибка при создании ТИмпорт\n\t%v", ош)
	}
	return секции, nil
}
