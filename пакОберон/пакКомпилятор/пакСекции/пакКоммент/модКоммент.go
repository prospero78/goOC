...
func (сам *ТКоммент) Обработать(пСловаМодуля []*мСлово.ТСлово) (ош error) {
	...
	for цНомерИсх = 0; цНомерИсх < len(пСловаМодуля); цНомерИсх++ {
		...
		if ош != nil {
			return мФмт.Errorf("ТуКоммент.Обработать(): ошибка1 при проверке слова\n\t%v", ош)
		}
		if стрСлово == "(*" {
			цУровень++
			continue
		...
		if стрСлово, ош = слово.Строка(); ош != nil {
			return мФмт.Errorf("ТуКоммент.Обработать(): ошибка2 при проверке слова\n\t%v", ош)
		}
		if стрСлово == "*)" {
			цУровень--
			continue
		...
		//пакКонс.Конс.Отладить("Уровень комментов: " + мФмт.Sprintf("%v", цУровень))
		if стрСлово, ош = слово.Строка(); ош != nil {
			return мФмт.Errorf("ТуКоммент.Обработать(): ошибка3 при проверке слова\n\t%v", ош)
		}
