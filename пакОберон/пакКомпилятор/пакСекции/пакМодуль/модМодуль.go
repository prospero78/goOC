...
type ТМодуль struct {
	...
	имя    *пакСлово.ТСлово //Имя модуля из слов модуля
}
...
func (сам *ТМодуль) Обработать(пСловаМодуля []*пакСлово.ТСлово) (ош error) {

	пакКонс.Конс.Отладить("ТМодуль.Обработать()")
	...
	if ош := сам._МодульНачалоПроверить(); ош != nil {
		ош = пакФмт.Errorf("ТМодуль.Обработать(): ошибка при поиске начала модуля\n\t%v", ош)
		return ош
	}
	if ош := сам._ИмяПроверить(); ош != nil {
		ош = пакФмт.Errorf("ТМодуль.Обработать(): ошибка при поиске имени модуля\n\t%v", ош)
		return ош
	}
	if ош := сам._РазделительПроверить(); ош != nil {
		ош = пакФмт.Errorf("ТМодуль.Обработать(): ошибка при поиске разделителя заголовка модуля\n\t%v", ош)
		return ош
	}
	if ош := сам._МодульКонецНайти(); ош != nil {
		ош = пакФмт.Errorf("ТМодуль.Обработать(): ошибка при поиске конца модуля\n\t%v", ош)
		return ош
	}
	if ош := сам._МодульОдинПроверить(); ош != nil {
		ош = пакФмт.Errorf("ТМодуль.Обработать(): ошибка при поиске единственного MODULE\n\t%v", ош)
		return ош
	}
...
func (сам *ТМодуль) _ИмяПроверить() (ош error) {
	пакКонс.Конс.Отладить("ТМодуль._ИмяПроверить()")
...
func (сам *ТМодуль) СловаСекцииУст(слова []*пакСлово.ТСлово) {
	сам.секция.СловаУст(слова)
}

// СловаМодуля -- возвращает список слов модуля
func (сам *ТМодуль) СловаМодуля() (слова []*пакСлово.ТСлово) {
