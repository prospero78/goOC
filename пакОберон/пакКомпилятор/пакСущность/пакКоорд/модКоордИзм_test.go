...
func TestСтр1и(тест *пакТест.T) {
	ки, ош := НовыйИзм(1, 0)
	if ки == nil {
		тест.Errorf("Пустая ссылка при вызове пакКоорд.НовыйИзм()")
	}
	if ош != nil {
		тест.Errorf("Ошибка при создании *ТуКоордИзм\n %v", ош)
	}
	if ки.Стр() != 1 {
		тест.Errorf("Значение ТуКоордИзм.Стр()=1 не равно 1 (минимальное для номера строки)")
	}
}

// Тест на сопротивление 0-строке (должна быть минимум 1)
func TestСтр2и(тест *пакТест.T) {
	ки, ош := НовыйИзм(0, 0)
	if ош == nil {
		тест.Errorf("Неверное создание *ТуКоордИзм: строка не может иметь № 0")
	}
	if ош == nil && ки.Стр() == 0 { // При правильной работе прокатывать не должно
...
func TestСтр3и(тест *пакТест.T) {
	...
	if ош == nil && ки.Стр() == -1 { // При правильной работе прокатывать не должно
...
func TestПоз1и(тест *пакТест.T) {
	...
	if ош == nil && ки.Поз() != 10 { // При правильной работе прокатывать не должно
...
func TestПоз2и(тест *пакТест.T) {
	...
	if ош == nil && ки.Поз() == -5 { // При правильной работе прокатывать не должно
...
func TestПоз3и(тест *пакТест.T) {
	...
	ки.Поз_Доб()
	ки.Поз_Доб()
	if ки.Поз() != 5 { // При правильной работе прокатывать не должно
...
func TestПоз4и(тест *пакТест.T) {
	...
	ош = ки.Поз_Уст(-20)
	if ош == nil { // При правильной работе прокатывать не должно
...
func TestПоз5и(тест *пакТест.T) {
	...
	ош = ки.Поз_Уст(21)
	if ки.Поз() != 21 { // При правильной работе прокатывать не должно
