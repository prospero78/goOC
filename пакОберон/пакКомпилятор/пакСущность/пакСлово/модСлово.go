...
func Новое(пКоорд *пакКоорд.ТуКоордФикс, пСтрока, пСтрИсх string) (слово *ТуСлово, ош error) {
...
func (сам *ТуСлово) Строка() (строка string, ош error) {
	if сам.строка == "" {
		ош = пакФмт.Errorf("ТуСлово.Строка(): ТуСтрока.строка не может быть пустой")
		return "", ош
	}
...
func (сам *ТуСлово) СтрИсх() (стр string, ош error) {
	if сам.стрИсх == "" {
