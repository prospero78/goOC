...
type ТСлово struct {
	...
	род    мТипы.ТРод         // род слова
	Коорд  *мКоорд.ТКоордФикс // Координаты слова в исходном тексте
	номер  int                // Текущий номер слова
}

//Новое -- возвращает нвое слово
func Новое(пКоорд *мКоорд.ТКоордФикс, пСтрока, пСтрИсх string) (слово *ТСлово, ош error) {
	//Проверяет входящую строку на минимальную длину.
	словоПроверить := func(пСтрока string) (ош error) { // Вспомогательная функция
		if len(пСтрока) == 0 {
			return мФмт.Errorf(" Строка имеет длину 0\n")
		}
		return nil
	}

	родПроверить := func(сам *ТСлово, пСтрока string) (род мТипы.ТРод, ош error) { //Устанавливает род слова.
		ЕслиЦифра:=func(пЛит string)bool{
		...
		case пСтрока[0] == '"' && пСтрока[len(пСтрока)-1] == '"':
			род = КСтрока
		default:
			ош = мФмт.Errorf("Не могу классифицировать строку, строка=" + string(пСтрока))
		}
		//сам.Род = пакТипы.ТРод(род)
		return мТипы.ТРод(род), nil
	}

	if ош = словоПроверить(пСтрока); ош != nil {
		return nil, мФмт.Errorf("ТуСлово.Новое(): при вызове словоПроверить()\n\t%v", ош)
	}
	род, ош := родПроверить(слово, пСтрока)
	if ош != nil {
		//слово.конс.Отладить("ТуСлово:" + пСтрока + пакФмт.Sprintf(" %d", слово.номер))
		return nil, мФмт.Errorf("пакСлово.Новое(): при вызове родПроверить()\n\t%v", ош)
	}
	словВсего++
	слово = &ТСлово{
		строка: пСтрока,
...
func (сам *ТСлово) Строка() (строка string, ош error) {
	if сам.строка == "" {
		return "", мФмт.Errorf("ТСлово.Строка(): ТуСтрока.строка не может быть пустой\n")
	}
...
func (сам *ТСлово) СтрИсх() (стр string, ош error) {
	if сам.стрИсх == "" {
		return "", мФмт.Errorf("ТуСлово.СтрИсх(): строка исходника не может быть пустой")
	}
